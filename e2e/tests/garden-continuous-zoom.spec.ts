...
import { GARDEN_DETAIL_WHEEL_ZOOM_STEP, GARDEN_WHEEL_ZOOM_STEP } from "../../src/utils/wheelZoom";
...
test.describe("Garden continuous world zoom", () => {
  ...
  test("unselected agent crosses detail bands with fixed world bounds and reversible pixel growth", async ({ page }, testInfo) => {
    ...
    while ((await box(cell)).width < 800 && steps < 100) {
      ...
      // Each notch magnifies about the pointer, including the entry boundary.
      expect(after.width / before.width).toBeCloseTo(GARDEN_WHEEL_ZOOM_STEP, 2);
      expect(after.x).toBeCloseTo(x + (before.x - x) * GARDEN_WHEEL_ZOOM_STEP, 0);
      expect(after.y).toBeCloseTo(y + (before.y - y) * GARDEN_WHEEL_ZOOM_STEP, 0);
    ...
    for (let index = 0; index < steps; index++) {
      ...
      expect(after.width / before.width).toBeCloseTo(1 / GARDEN_WHEEL_ZOOM_STEP, 2);
      await expect(cell).toHaveAttribute("data-garden-world", world!);
    }
  ...
  test("selected memory reading plane grows around its world anchor and reverse wheel restores agent", async ({ page }) => {
    ...
    while ((await box(cell)).width < 750 && steps < 100) {
      const { before, after, x, y } = await wheel(page, cell, -120);
      expect(after.width / before.width).toBeCloseTo(GARDEN_DETAIL_WHEEL_ZOOM_STEP, 2);
      expect(after.x).toBeCloseTo(x + (before.x - x) * GARDEN_DETAIL_WHEEL_ZOOM_STEP, 0);
      // The reading plane expands vertically about the source anchor, not its top edge.
      expect(after.y + after.height / 2).toBeCloseTo(y + (before.y + before.height / 2 - y) * GARDEN_DETAIL_WHEEL_ZOOM_STEP, 0);
