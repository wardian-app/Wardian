...
export const VariableAssistant = memo(({ selectedNodeId }: VariableAssistantProps) => {
      ...
      <div key={key.path} className="space-y-0.5">
        ...
        >
          ...
          {copiedPath === `${baseId}.${key.path}` ? (
            <span className={`text-[8px] font-bold animate-in fade-in slide-in-from-right-1 duration-200`} style={{ color: accentColor }}>COPIED</span>
          ) : (
  ...
  const renderNodeTree = (node: any, isPrevious: boolean = false) => {
    ...
    const blockName = blockDef?.name || node.type?.toUpperCase();
        ...
        >
          <div className="flex flex-col">
            ...
            <span className="text-[8px] font-medium text-[var(--color-wardian-text-muted)] opacity-70">
              {blockName} {isPrevious && <span className="text-[var(--color-wardian-warning)] font-bold italic ml-1 tracking-tighter">(PREV ITER)</span>}
            </span>
          ...
          {copiedPath === baseId ? (
            <span className={`text-[9px] font-bold ${isPrevious ? 'text-[var(--color-wardian-warning)]' : 'text-[var(--color-wardian-accent)]'} animate-in fade-in slide-in-from-right-1 duration-200`}>COPIED</span>
          ) : (
    ...
    >
      <div className="p-4 border-b border-wardian-border bg-white/5">
        <div className="flex items-center justify-between mb-3">
          <span className="text-[10px] font-black text-muted-neutral uppercase tracking-[0.2em]">Variable Assistant</span>
        </div>
        <input 
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 no-scrollbar min-h-0">
        ...
        <section className="space-y-2">
          <h4 className="text-[9px] font-bold text-muted-neutral uppercase tracking-widest px-1">Upstream Outputs</h4>
        ...
        {upstreamContext.previousNodes.length > 0 && (
          <section className="space-y-2">
            <h4 className="text-[9px] font-bold text-[var(--color-wardian-warning)] uppercase tracking-widest px-1 opacity-80">Cycle Contexts</h4>
            <div className="space-y-1.5">
        ...
        <section className="space-y-2 pt-4 border-t border-white/5">
          <h4 className="text-[9px] font-bold text-muted-neutral uppercase tracking-widest px-1">Shared Storage</h4>
          <div 
