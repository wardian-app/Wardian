...
function openMarkdownUrl(
  ...
  linkHandling?: ChatMarkdownProps["linkHandling"],
) {
  event.preventDefault();
  event.stopPropagation();
  const href = event.currentTarget.getAttribute("href") ?? event.currentTarget.href;
  if (linkHandling?.getExternalEditor) {
    void resolveTerminalLinkTarget(href, linkHandling).then((link) => {
      if (link) {
        openTerminalDetectedLink(link, {
          getExternalEditor: linkHandling.getExternalEditor,
          onOpenError: linkHandling.onOpenError,
          openFile: linkHandling.openFile,
          openUrl: linkHandling.openUrl,
        });
        return;
      }
...
function createComponents(linkHandling?: ChatMarkdownProps["linkHandling"]): Components {
  return {
    a({ href, children }) {
      ...
      return (
        <a
          ...
          href={safeUrl}
          onClick={(event) => openMarkdownUrl(event, linkHandling)}
          rel="noreferrer"
          target="_blank"
        >
          {children}
...
export function ChatMarkdown({ linkHandling, source }: ChatMarkdownProps) {
  return (
    <div className="space-y-2 text-[13px] leading-5 text-primary">
      <Markdown components={createComponents(linkHandling)} remarkPlugins={[[remarkGfm, { singleTilde: false }]]} skipHtml urlTransform={markdownUrlTransform}>
        {source}
      </Markdown>
    </div>
  );
