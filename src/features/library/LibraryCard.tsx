...
export const LibraryCard: React.FC<LibraryCardProps> = ({ prompt, hasSelectedAgents, onClick, onToggleStar, onRun }) => {
    return (
        <div 
            ...
            <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-1">
                    {hasSelectedAgents && (
                        <button 
                            onClick={onRun}
                            className="p-1.5 rounded-md text-wardian-success opacity-0 group-hover:opacity-100 transition-all hover:bg-wardian-success/10 z-10"
                            title="Send to Selected Agent(s)"
                        >
                    ...
                    <button 
                        ...
                        className={`p-1.5 rounded-md transition-colors z-10 ${prompt.metadata.is_starred ? 'text-yellow-400 hover:text-yellow-300' : 'text-muted-neutral opacity-0 group-hover:opacity-100 hover:text-primary'}`}
                    >
