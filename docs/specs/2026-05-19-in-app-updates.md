...
The release workflow signs update artifacts with `TAURI_SIGNING_PRIVATE_KEY` and optional `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` secrets. Parallel matrix jobs upload updater bundles and loose signatures without writing shared metadata. After the matrix succeeds, one job embeds every updater signature in `latest.json`, replaces that release asset once, and then deletes the loose `.sig` assets before publication. The updater consumes inline signatures from `latest.json`, not separate signature URLs. The public key is committed in `tauri.conf.json`; the private key never appears in the repository or documentation beyond secret-name references.
