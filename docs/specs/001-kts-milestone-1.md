...
> Note: This proposal is partially superseded by [Spec 015](./015-evidence-first-memory.md). KTS remains valid as a promoted knowledge layer, but it is no longer the recommended base memory substrate.
...
Under [Spec 015](./015-evidence-first-memory.md), this projection should consume both promoted atoms and raw evidence, with atoms serving as higher-confidence curated records rather than the first ingestion layer.
