...
- Already-mounted Codex terminals receive the same mode and color refresh when Wardian's effective theme changes, then repaint the serialized xterm state through the active theme and explicitly repaint the visible composer block around the cursor with Codex's active composer fill. A retained PTY preview is only a fallback when no local terminal state is available. Switching back to dark mode should not require restarting the agent or typing into the composer.
