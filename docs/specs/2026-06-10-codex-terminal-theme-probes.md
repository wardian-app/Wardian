...
- Already-mounted Codex terminals receive the same mode and color refresh when Wardian's effective theme changes, then repaint the serialized xterm state through the active theme and force a renderer refresh. A retained PTY preview is only a fallback when no local terminal state is available. Switching back to dark mode should not require restarting the agent or typing into the composer.
