...
Wardian provides one orchestration layer over five supported CLI providers: Antigravity, Claude Code, Codex, OpenCode, and Pi. Legacy Gemini CLI integration remains visible as unmaintained compatibility code. Each provider keeps its native command-line behavior, while Wardian adapts session identity, working roots, skill discovery, status tracking, and automation execution into a consistent app model.
...
| **[Claude Code](https://github.com/anthropics/claude-code)** | Supported | Real target workspace | `AGENTS.md` | `--add-dir` instruction roots plus `.claude/skills` links to Wardian-managed skills | Wardian assigns fresh session IDs and resumes explicitly |
...
| **[Pi](https://pi.dev/docs/latest/)** | Supported | Real target workspace for new sessions; saved project path on resume | `AGENTS.md` | Wardian instruction files use `--append-system-prompt`; skill roots use repeated `--skill` | Wardian assigns an exact project session ID and resumes explicitly |
| **[Gemini CLI](https://github.com/google-gemini/gemini-cli)** | Unmaintained | Real target workspace | Legacy `GEMINI.md`; not generated by Wardian | Wardian include roots passed through `--include-directories`; Gemini patch enables multi-root skill discovery | Discovered from provider output |
...
Claude reads `AGENTS.md` natively. Wardian enables additional-directory discovery and maintains `.claude/skills` links where needed so Claude can see Wardian-managed common, class, and agent instructions and skills without provider-specific instruction copies.

`AGENTS.md` is the only Wardian-generated instruction file. During home and
habitat initialization, Wardian removes exact legacy `@AGENTS.md` stubs and
unchanged hash-marked Claude projections from its managed common, class, agent,
and habitat roots. Customized files, symbolic links, junctions, and hardlinks
are preserved. Workspace files and user-selected include directories are never
rewritten by this migration.

Wardian also launches Claude-managed terminal surfaces with Claude Code's alternate-screen opt-out enabled. This preserves native terminal scrollback for desktop terminals and mobile PWA drag scrolling while keeping Claude's `AGENTS.md` discovery behavior.
...
screen. A user-authored external `CLAUDE.md` import consent menu is `Action Needed`, not an
...
New Pi sessions normally run in the real target workspace. Pi keeps the user's
global configuration, authentication, packages, extensions, and themes.
...
When resuming, Wardian starts Pi from the project path recorded in that exact
session's header if it belongs to the agent's real workspace or habitat. This
avoids Pi's prompt to fork the session into a different project. If the session
has no JSONL yet, Wardian uses the fresh-launch path. On Windows, a workspace
path beyond the terminal launch limit uses the agent's short habitat path; a
saved path beyond that limit produces an explicit restore error because a new
alias would change Pi's project identity.
...
Gemini's legacy instruction contract reads `GEMINI.md`, but Wardian no longer generates that compatibility file. Wardian still passes common, class, and agent include roots through `--include-directories`; this path is unmaintained and may require user-supplied provider configuration. The Gemini skill patch lets the CLI discover skills from those additional roots rather than only from the global or project-local Gemini skill folders.
