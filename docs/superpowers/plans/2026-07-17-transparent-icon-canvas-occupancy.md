...
**Architecture:** Generate an enlarged 512px transparent master from \`public/icon-transparent.png\`, then regenerate the transparent Tauri icon family with the repository's Tauri CLI. Keep the existing 16px and 24px PNG/ICO frames because their current one-pixel edge buffer leaves no space for a larger mark.
...
- [ ] **Step 2: Create the enlarged master and regenerate into an isolated directory**
...
result.save(r'''$tempMaster''')
'@.Replace('$tempMaster', $tempMaster.Replace('\', '\\')) | python -
...
- [ ] **Step 3: Copy only the transparent generated outputs, then restore the two source-limited PNG files**
...
- [ ] **Step 4: Restore the matching 16px and 24px ICO frames**

Before generation, save the original \`icon.ico\` to \`$tempRoot/original.ico\`. Then run:

\`\`\`powershell
@'
from PIL import Image

original = Image.open(r'''$tempRoot/original.ico''')
