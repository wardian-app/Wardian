...
- [x] **Step 1: Run browser E2E on pull requests** and add a Windows targeted `native-workbench-smoke` job using an explicit runner-temp Wardian home. Upload native artifacts on failure. Keep full native coverage local/manual if CI duration requires, but the persistence/broker/lifecycle focused suite is required.
