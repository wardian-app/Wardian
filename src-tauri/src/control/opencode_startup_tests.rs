...
async fn message_delivery_writes_terminal_bytes_after_opencode_startup_is_ready() {
    ...
    let delivered = delivery.await.unwrap();
    assert_eq!(delivered[0].delivery_state, "provider_accepted");
    assert_eq!(delivered[0].delivery_phase.as_deref(), Some("turn_started"));
