...
impl Row {
    ...
    pub fn write_contents_diff(
                ...
                >= self.cols() - u16::from(prev_first_cell.is_wide())
        {
            ...
            let need_erase = if prev_first_cell.has_contents() {
                false
            } else {
                true
            };
            if need_erase {
                ...
                write_hyperlink_transition(
                    contents,
                    &mut active_hyperlink,
                    prev_first_cell.hyperlink(),
                );
                prev_first_cell.write_contents(contents);
            }
            crate::term::Backspace.write_buf(contents);
        ...
        if (!self.wrapped && prev.wrapped) || (!prev.wrapped && self.wrapped)
        {
            ...
            if end_cell.has_contents() {
                ...
                end_cell.write_contents(contents);
...
fn write_hyperlink_transition<'a>(
    contents: &mut Vec<u8>,
    ...
    if let Some(uri) = target {
        crate::term::write_hyperlink_start(contents, uri);
    }
    *active = target;
}

fn finish_hyperlink<'a>(contents: &mut Vec<u8>, active: &mut Option<&'a str>) {
    if active.is_some() {
        crate::term::write_hyperlink_end(contents);
        *active = None;
    }
