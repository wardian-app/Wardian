...
impl<CB: crate::callbacks::Callbacks> vte::Perform for WrappedScreen<CB> {
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], _bel_terminated: bool) {
        if params.first().is_some_and(|param| *param == b"8") {
            self.screen.set_hyperlink(
                parse_hyperlink_uri(params)
                    .as_deref()
...
const MAX_HYPERLINK_PARAMS_BYTES: usize = 128;
const MAX_HYPERLINK_OSC_PARAMS: usize = 16;

fn parse_hyperlink_uri(params: &[&[u8]]) -> Option<String> {
    if params.len() < 3 || params.len() >= MAX_HYPERLINK_OSC_PARAMS {
        return None;
    }
    if !valid_hyperlink_bytes(params[1], MAX_HYPERLINK_PARAMS_BYTES) {
        return None;
    }
    ...
    for (index, part) in params[2..].iter().enumerate() {
        ...
        if !valid_hyperlink_bytes(part, crate::cell::MAX_HYPERLINK_URI_BYTES) {
            return None;
        }
        uri.extend_from_slice(part);
        if uri.len() > crate::cell::MAX_HYPERLINK_URI_BYTES {
            return None;
        }
    }
    if uri.is_empty() {
...
fn valid_hyperlink_bytes(bytes: &[u8], max_bytes: usize) -> bool {
    bytes.len() <= max_bytes
