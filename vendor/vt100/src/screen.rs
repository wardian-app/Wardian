...
pub struct Screen {
    ...
    active_hyperlink: Option<Arc<crate::cell::HyperlinkTarget>>,
    saved_hyperlink: Option<Arc<crate::cell::HyperlinkTarget>>,
    hyperlink_budget: Arc<crate::cell::HyperlinkBudget>,
...
impl Screen {
    pub(crate) fn new(
        ...
        Self {
            ...
            hyperlink_budget: Arc::new(crate::cell::HyperlinkBudget::default()),
    ...
    pub fn state_formatted(&self) -> Vec<u8> {
        ...
        // A snapshot may be applied after a link-bearing stream. Close any
        // target that the consumer still has active before restoring cells.
        crate::term::write_hyperlink_end(&mut contents);
    ...
    fn write_contents_diff(&self, contents: &mut Vec<u8>, prev: &Self) {
        ...
        // Row writers close links at their boundaries; clear the link left by
        // the previous screen before emitting the first changed row.
        if prev.active_hyperlink.is_some() {
            crate::term::write_hyperlink_end(contents);
        }
    ...
    fn save_cursor(&mut self) {
        ...
        self.saved_hyperlink = self.active_hyperlink.clone();
    }

    fn restore_cursor(&mut self) {
        self.grid_mut().restore_cursor();
        self.attrs = self.saved_attrs;
        self.active_hyperlink = self.saved_hyperlink.clone();
    }

    pub(crate) fn set_hyperlink(&mut self, uri: Option<&str>) {
        self.active_hyperlink = uri.and_then(|uri| {
            crate::cell::HyperlinkTarget::new(uri, &self.hyperlink_budget)
        });
    }

    fn write_active_hyperlink_formatted(&self, contents: &mut Vec<u8>) {
        if let Some(hyperlink) = self.active_hyperlink.as_ref() {
            crate::term::write_hyperlink_start(contents, hyperlink.uri());
        }
    }
...
impl Screen {
    pub(crate) fn text(&mut self, c: char) {
        ...
        // Keep the shared target alive without borrowing the screen while the
        // cursor and grid are mutably updated below.
        ...
        if width == 0 {
            ...
            let cell = self
                ...
                // that self.grid().pos().col has a valid value.
                .unwrap();
            cell.set_with_hyperlink(c, attrs, hyperlink.as_ref());
            ...
            if width > 1 {
                ...
                next_cell.set_hyperlink(hyperlink.as_ref());
